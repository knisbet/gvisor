...
func NewDurationBucketer(numFiniteBuckets int, minDuration, maxDuration time.Duration) Bucketer {
	...
	exponentCoversNs := float64(maxDuration.Nanoseconds()-int64(numFiniteBuckets-3)*minNs) / float64(minNs)
	exponent := math.Log(exponentCoversNs) / math.Log(float64(numFiniteBuckets-3))
...
func (o TimedOperation) Finish(extraFields ...string) {
	...
	o.metric.addSampleByKey(CheapNowNano()-o.startedNs, fieldKey)
}
